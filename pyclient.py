...
def on_message(ws, message, num=[0,0,0,0,0], r=skeleton()):
	...
	if words[0][1:-1] == "/NI_mate_sync":
		...
		num[3] = 1
	elif words[0][1:-1] == "Head":
		r.joints[4].joint_name = words[0][1:-1]
		r.joints[4].x = words[1]
		r.joints[4].y = words[2]
		r.joints[4].z = words[3]
		r.joints[4].stamp = rospy.get_time()
		num[4] = 1
...
if __name__ == '__main__':
	...
	try:
		...
		ws = websocket.WebSocketApp("ws://130.215.206.207:3000", on_message=on_message, on_error=on_error, on_close=on_close)
		ws.on_open = on_open
		ws.run_forever()
